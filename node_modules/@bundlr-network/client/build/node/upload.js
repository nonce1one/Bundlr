...
class NodeUploader extends upload_1.default {
    ...
    async uploadFile(path, opts) {
        ...
        (opts !== null && opts !== void 0 ? opts : (opts = {})).tags = [{ name: "Content-Type", value: (_a = this.contentTypeOverride) !== null && _a !== void 0 ? _a : mimeType }, ...((_b = opts === null || opts === void 0 ? void 0 : opts.tags) !== null && _b !== void 0 ? _b : [])];
    ...
    * @returns
    ...
    async uploadFolder(path, { batchSize = 10, keepDeleted = true, indexFile, interactivePreflight, logFunction } = { batchSize: 10, keepDeleted: true }) {
        path = p.resolve(path);
        ...
        const processor = async (data) => {
            var _a;
            if ((_a = data === null || data === void 0 ? void 0 : data.res) === null || _a === void 0 ? void 0 : _a.id) {
        ...
        const mres = await this.uploadData((0, fs_1.createReadStream)(jsonManifestPath), { tags })
            .catch((e) => {
            throw new Error(`Failed to upload manifest: ${e.message}`);
        });
        await logFunction("Done!");
        if (mres === null || mres === void 0 ? void 0 : mres.id) {
            await fs_1.promises.writeFile(p.join(p.join(path, `${p.sep}..`), `${p.basename(path)}-id.txt`), JSON.stringify(mres));
        }
        return mres;
    }
    /**
    ...
    async processItem(item) {
        var _a;
        if (arbundles_1.DataItem.isDataItem(item)) {
